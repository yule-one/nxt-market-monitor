...
EMPLOYEE_NUMBER_PATTERN = re.compile(r"^[0-9]{6}$")
...
def normalize_employee_number(employee_number: str) -> str:
    normalized = str(employee_number or "").strip()
    if not EMPLOYEE_NUMBER_PATTERN.fullmatch(normalized):
        raise AuthError("사번은 숫자 6자리로 입력하세요.")
    return normalized
...
class AuthStore:
    ...
    def _valid_session_token(token: str) -> bool:
        return bool(re.fullmatch(r"[A-Za-z0-9_-]{40,128}", str(token or "")))
    ...
    def create_persistent_session(
        ...
        *,
        ...
        now_text = self._serialize_time(now)
        expires_text = self._serialize_time(expires_at)
        with self._lock, self._connect() as connection:
            ...
            if (
                ...
                raise AuthError("로그인 세션을 만들 수 없는 계정입니다.")
            connection.execute(
            ...
            connection.execute(
                ...
                """,
                (int(user_id), token_hash, now_text, expires_text, now_text),
            )
            self._write_audit(
                connection,
                ...
                detail=f"expires_at={expires_text}",
            )
        return raw_token, expires_at

    def authenticate_persistent_session(self, token: str) -> AuthUser | None:
        if not self._valid_session_token(token):
            return None
        ...
        with self._lock, self._connect() as connection:
            ...
            if (
                ...
                return None
            last_seen_at = self._parse_time(session["last_seen_at"])
            if last_seen_at is None or now - last_seen_at >= timedelta(minutes=5):
                connection.execute(
                    "UPDATE auth_sessions SET last_seen_at = ? WHERE id = ?",
                    (now_text, int(session["id"])),
                )
    ...
    def reset_password(
        ...
        *,
        ...
        with self._lock, self._connect() as connection:
            ...
            self._revoke_user_sessions(connection, int(target_user_id))
            self._write_audit(
                connection,
                actor_username=str(actor["username"]),
